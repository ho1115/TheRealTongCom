...
class hisDataCarrier {
    ...
    async chapters(bookName) {
        var result = []
        if (bookName === 'TongZhi') {
            var sql = 'SELECT DISTINCT chapter FROM tongzhi'
            const qRes = await conn.promise().query(sql)
            for (let i = 0; i < qRes[0].length; i++) {result.push(qRes[0][i]['chapter'])}
        } else {
            var sql = 'SELECT volumn FROM official_history where book_name = ?'
            const qRes = await conn.promise().query(sql, [bookName])
            for (let i = 0; i < qRes[0].length; i++) {result.push(qRes[0][i]['volumn'])}
        }
        conn.end()
        console.log(result)
        return result
    ...
    async subChaps(bookName, chapName, version) {
